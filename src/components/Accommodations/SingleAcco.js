...
const SingleAccommodation = (props) => {
  const { accommodation, place } = props;
  const location = useLocation();
  console.log('Place ', place);
  return (
    <div className="flex flex-col gap-4 pb-4 border-4 border-slate-50 border-b-[#e395b7] rounded-lg relative">
      <div>
        <img src="https://picsum.photos/200" alt="" className="w-full h-52" />
      </div>
      <img src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541" alt="" className="rounded-full absolute h-12 bottom-[37%] left-3/4" />
      <div className="px-4 text-start flex flex-col gap-4 mt-5">
        <h3>{`${accommodation.description.slice(0, 30)}...`}</h3>
        <div className="flex gap-2 text-sm text-gray-500">
          <img src={pinIcon} className="h-4" alt="" />
          <p>
            Country:
            {' '}
            {place[0].country}
      ...
      {location.pathname.includes('/accommodations/delete') ? <button type="button">Delete</button> : ''}
    </div>
  );
};
